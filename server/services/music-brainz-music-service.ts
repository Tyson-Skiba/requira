...
import type { ArtistCredit, MusicBrainzResponse, MusicBrainzSearchResult } from '../../models/songs/music-brainz';

export class MusicBrainzProvider {

    search = async (title: string, artist: string): Promise<MusicBrainzSearchResult[]> => {
        const url =
      ...
      const results = await Promise.all(
        recordings.map(async rec => {
          const artistCredit = (rec["artist-credit"]?.[0]?.artist || {}) as ArtistCredit;
